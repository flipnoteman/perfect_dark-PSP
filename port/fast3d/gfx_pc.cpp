...
static float buf_vbo[MAX_BUFFERED * (10 * 3)]; // 3 verts × 9 floats/vert
...
static void import_texture_rgba16(int tile, const LoadedTexture &loaded_texture,
  ...
  const uint8_t *src =
      fix_npot_texture((const uint8_t *)tex_upload_buffer, width, height, pot_w,
                       pot_h, temp_pot_buf);
  // ------------------------------------------------------------------
  ...
  const uint8_t *src_mirrored =
      mirror_texture_rgba32(src, /* POT‑corrected source pixels     */
                            pot_w, pot_h, mirror_s, mirror_t, mirror_buf, pot_w,
                            pot_h); /* may double w/h */
...
static void import_texture_rgba32(int tile, const LoadedTexture &loaded_texture,
  ...
  const uint8_t *src_mirrored = mirror_texture_rgba32(
      src8, pot_w, pot_h, mirror_s, mirror_t, mirror_buf, pot_w, pot_h);
  src8 = src_mirrored;
...
static void import_texture_ia4(int tile, const LoadedTexture &loaded_texture,
  ...
  std::vector<uint8_t> mirror_buf;
  const uint8_t *src_mirrored = mirror_texture_rgba32(
      src, pot_w, pot_h, mirror_s, mirror_t, mirror_buf, pot_w, pot_h);
  src = src_mirrored;
  gfx_rapi->upload_texture(src, pot_w, pot_h);
  if (rendering_state.textures[0]) {
    rendering_state.textures[0]->second.pot_w = pot_w;
...
static void split_if_needed(const TempV &A, const TempV &B, const TempV &C,
  ...
  if (depth == 0 || wmax / wmin < threshold) {
    emit(A, B, C);
    return;
  }
  ...
  if (scrDist2(B, C) > scrDist2(*P, *Q)) {
    P = &B;
    Q = &C;
    R = &A;
  }
  if (scrDist2(C, A) > scrDist2(*P, *Q)) {
    P = &C;
    Q = &A;
    R = &B;
  }
...
static void gfx_sp_tri1(uint8_t vtx1_idx, uint8_t vtx2_idx, uint8_t vtx3_idx,
  ...
  static const float clip_plane[4]
      __attribute__((aligned(16))) = {0.0f, 0.0f, -1.0f, -1.0f};
  ...
  if ((rsp.geometry_mode & G_CULL_BOTH) != 0) {
    ...
    if ((v1->w < 0) ^ (v2->w < 0) ^ (v3->w < 0)) {
      // If one vertex lies behind the eye, negating cross will give the correct
      // result. If all vertices lie behind the eye, the triangle will be
      // rejected anyway.
      cross = -cross;
    }
  ...
  uint16_t zmode =
      (rdp.other_mode_l & ZMODE_DEC) >> 6; // extract zmode bits properly
  ...
  if (rdp.viewport_or_scissor_changed) {
    ...
    if (memcmp(&rdp.scissor, &rendering_state.scissor, sizeof(rdp.scissor)) !=
        0) {
      gfx_flush();
  ...
  bool use_alpha = (rdp.other_mode_l & (3 << 20)) == (G_BL_CLR_MEM << 20) &&
  ...
  const bool use_noise =
      (rdp.other_mode_l & (3U << G_MDSFT_ALPHACOMPARE)) == G_AC_DITHER;
  const bool use_2cyc =
      (rdp.other_mode_h & (3U << G_MDSFT_CYCLETYPE)) == G_CYC_2CYCLE;
  const bool alpha_threshold =
      (rdp.other_mode_l & (3U << G_MDSFT_ALPHACOMPARE)) == G_AC_THRESHOLD;
  const bool invisible = (rdp.other_mode_l & (3 << 24)) == (G_BL_0 << 24) &&
                         (rdp.other_mode_l & (3 << 20)) == (G_BL_CLR_MEM << 20);
  const bool use_grayscale = rdp.grayscale;
  const bool use_modulate =
      use_alpha && (rsp.extra_geometry_mode & G_MODULATE_EXT) != 0;
  const bool use_blur =
      (rdp.other_mode_h & (3U << G_MDSFT_TEXTFILT)) == G_TF_BLUR_EXT;
  ...
  // If we are not using alpha, clear the alpha components of the combiner as
  // they have no effect
  if (!use_alpha) {
  ...
  for (int i = 0; i < 2; i++) {
    ...
    if (comb->used_textures[i]) {
      ...
      tex_width2[i] =
          (rdp.texture_tile[tile].lrs - rdp.texture_tile[tile].uls + 4) / 4;
      tex_height2[i] =
          (rdp.texture_tile[tile].lrt - rdp.texture_tile[tile].ult + 4) / 4;

      uint32_t tex_width1 = tex_width[i] << (cms & G_TX_MIRROR);
      uint32_t tex_height1 = tex_height[i] << (cmt & G_TX_MIRROR);

      if ((cms & G_TX_CLAMP) &&
          ((cms & G_TX_MIRROR) || tex_width1 != tex_width2[i])) {
        tm |= 1 << 2 * i;
        cms &= ~G_TX_CLAMP;
      }
      if ((cmt & G_TX_CLAMP) &&
          ((cmt & G_TX_MIRROR) || tex_height1 != tex_height2[i])) {
        tm |= 1 << (2 * i + 1);
        cmt &= ~G_TX_CLAMP;
      }
      ...
      if (rendering_state.textures[i]) {
        ...
        if (linear_filter !=
                rendering_state.textures[i]->second.linear_filter ||
  ...
  if (use_alpha != rendering_state.alpha_blend ||
      use_modulate != rendering_state.modulate) {
    gfx_flush();
